...
$metaboxes = array(
    'link' => array(
        ...
        'fields' => array(
            'link_title' => array(
                'title' => __('Link Title:', 'dalux'),
                'type' => 'text',
                'description' => '',
                'size' => 60
            ),
            'link_url' => array(
                'title' => __('link url:', 'dalux'),
                'type' => 'text',
                'description' => '',
                'size' => 60
            )
...
add_action( 'add_meta_boxes', 'dalux_add_post_format_metabox' );

function dalux_add_post_format_metabox() {
    global $metaboxes;

    if ( ! empty( $metaboxes ) ) {
        foreach ( $metaboxes as $id => $metabox ) {
            add_meta_box( $id, $metabox['title'], 'dalux_show_metaboxes', $metabox['applicableto'], $metabox['location'], $metabox['priority'], $id );
        }
    }
}

function dalux_show_metaboxes( $post, $args ) {
    global $metaboxes;
...
add_action( 'save_post', 'dalux_save_metaboxes' );

function dalux_save_metaboxes( $post_id ) {
    global $metaboxes;
...
add_action( 'admin_print_scripts', 'dalux_display_metaboxes', 1000 );
function dalux_display_metaboxes() {
    global $metaboxes;
...
add_filter( 'cmb_meta_boxes', 'dalux_cmb_metaboxes' );
function dalux_cmb_metaboxes( array $meta_boxes ) {

    $prefix = 'dalux_';

     $meta_boxes['page_metabox'] = array(
        'id'         => 'page_metabox',
        'title'      => __( 'Creativ Page Settings', 'dalux' ),
        ...
        'fields'     => array(
            array(
                ...
                'name'    => __( 'Select Template Background Color','dalux'),
                'desc' => __('Background color of selected template will change accordingly except builder templates', 'dalux') ,
                'type'    => 'select',
                'options' => array(
            ...
            array(
                ...
                'name'    => __( 'Page Breadcrumb Options','dalux'),
                'type'    => 'select',
                'options' => array(
            ...
            array(
                'name' => __('Background Breadcrumb Image', 'dalux') ,
                'desc' => __('Upload an image or enter a URL. This works only for Breadcrumb Background Image style.', 'dalux') ,
            ...
            array(
                'name' =>  __( 'Hide Social Icons', 'dalux' ),
    ...
    $meta_boxes['post_metabox'] = array(
        'id'         => 'post_metabox',
        'title'      => __( 'Creativ Post Page Settings', 'dalux' ),
     ...
     $meta_boxes['portfolio_metabox'] = array(
        ...
        'title'      => __( 'Custom Portfolio Options', 'dalux' ),
    ...
    $meta_boxes['event_metabox'] = array(
            'id'         => 'event_metabox',
            'title'      => __( 'Custom Event Options', 'dalux' ),
            'pages'      => array( 'event' ),
            'context'    => 'normal',
            'normal'   => 'high',
            'show_names' => true, // Show field names on the left
     ...
     $meta_boxes['menu_metabox'] = array(
            'id'         => 'menu_metabox',
            'title'      => __( 'Custom Menu Options', 'dalux' ),
            'pages'      => array( 'menu' ),
            'context'    => 'normal',
            'normal'   => 'high',
            'show_names' => true, // Show field names on the left
